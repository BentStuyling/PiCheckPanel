...
SPD_GPIO = 26
# SPD_GPIO = 19
SPD_signl =  read_PWM.reader(pi, SPD_GPIO)
...
class Status(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(frame_row_7, font = StatusFont, fg =StatusValuecolor,
                 bg = 'black', textvariable = self.Humid).place(x = 140, y =15, anchor ='w')         
        tk.Label(frame_row_7, font = StatusFont, fg =StatusValuecolor,
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.ax1.tick_params(axis='y', labelcolor= Graphcolor, pad = 1, length = 0,  labelsize=14,)
        self.ax1.set_yticks([80,85,90,95,100, 105])
        self.ax1.grid(linewidth=2, axis = 'y')
        self.ax1.tick_params(axis='x', labelcolor=Graphcolor, pad = 1, labelsize = 14)
        self.ax1.set_xlabel('ZEIT [s]', color='white',labelpad= 0, weight = 'bold',fontsize= 13)
        ...
        self.fig.text(0.03, 0.05, 'KUHLW', color='tab:blue', fontsize=13, weight = 'bold')
        self.fig.text(0.87, 0.05, 'ÖL', color='tab:orange', fontsize=13, weight = 'bold')
        # Axis 2 (right)
        # color = 'tab:orange'
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.ax1.set_xticks([0 ,self.x_len/4, self.x_len/2, self.x_len/1.333, self.x_len])
        self.ax1.set_xticklabels([int(self.graph_duration),round(int(self.graph_duration)/1.3333),round(int(self.graph_duration)/2),round(int(self.graph_duration)/4),0])    
        self.fig.text(0.03, 0.05, 'ACHSE', color='tab:blue', fontsize=13, weight = 'bold')
        self.fig.text(0.68, 0.05, 'GETRIEBE', color='tab:orange', fontsize=13, weight = 'bold')
        # Axis 2 (right)
        # color = 'tab:orange'
        # self.ax2.set_ylabel('ÖLTEMP. [C]', color=color, weight = 'bold',fontsize= 13)
        # self.ax2.tick_params(axis='y', labelcolor=color, pad = 1, length = 0, labelsize=10)
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Parameters
        self.x_len = int(round(((self.graph_duration*1000)/self.update_interval),0)) # Number of points to display
        self.y1_range = [0, 130]  # Range of possible Y values to display left axis
        self.y2_range = [0, 20]  # Range of possible Y values to display right axis
        # Create graph fiugre
        self.fig = figure.Figure(facecolor= 'black', figsize=(3.25, 2),dpi=75)
        self.ax1 = self.fig.add_subplot(1, 1, 1,facecolor = 'black')
        ...
        self.ax1.tick_params(axis='y', labelcolor='white', pad = 1, length = 0,  labelsize=14,)
        self.ax1.set_yticks([0,25,50,75,100, 125])
        self.ax1.grid(linewidth=2, axis = 'y')
        self.ax1.tick_params(axis='x', labelcolor='white', pad = 1, labelsize = 14)
        self.ax1.set_xlabel('ZEIT [s]', color=Graphcolor,labelpad= 0, weight = 'bold',fontsize= 13)
        self.ax1.set_ylim(self.y1_range)
        self.ys1 = [0] * self.x_len
        self.xs = list(range(0, self.x_len))
        self.ax1.set_xticks([0 ,self.x_len/4, self.x_len/2, self.x_len/1.333, self.x_len])
        self.ax1.set_xticklabels([int(self.graph_duration),round(int(self.graph_duration)/1.3333),round(int(self.graph_duration)/2),round(int(self.graph_duration)/4),0])    
        self.fig.text(0.03, 0.05, 'GESCHW.', color='white', fontsize=13, weight = 'bold')
        self.fig.text(0.75, 0.05, 'VERBR.', color=Graphcolor, fontsize=13, weight = 'bold')
        #Axis 2 (right)
        #self.ax2.set_ylabel('VERBR. [KM/L]', color=Graphcolor, weight = 'bold',fontsize= 10)
        self.ax2.tick_params(axis='y', labelcolor=Graphcolor, pad = 1, length = 0, labelsize=14)
        self.ax2.set_yticks([0,5,10,15,20])
        self.ax2.set_ylim(self.y2_range)
        self.ys2 = [0] * self.x_len
        # PLot lines
        color = 'white'
    ...
    def GetValPos_4(self):
        ...
        self.list_leng = round(self.avg_window/(self.TimerInterval/1000)) #Calculate appropriate list length
...
class get_signal:
    ...
    def Speed(self):
        ...
        Correction = 0.98
        self.value = (SPD_signl.frequency()/9)*Wheelcircumf*3.6*Correction #km/h
        return self.value 
