...
class Status(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(frame_row_4, font = StatusFont, fg =StatusValuecolor,
                 bg = 'black', text = Update_date).place(x = 100, y =15, anchor ='w')     
    ...
    def Get_Temp(self):
        self.value = get_signal.GetModuleTemp(self)
        self.Temp.set(self.value)
        self.TimerInterval = 5000 # Update interval of this sensor value
        self.after(self.TimerInterval,self.Get_Temp)
    
    def Get_Humid(self):
        self.value = get_signal.GetModuleHumid(self)
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(frame_value_row_1,font = UnitFont, fg =Unitcolor,
                 bg = 'black', text = 'BAR').place(x = 180, y =19, anchor ='w')        
        ...
        #Battery Voltage:
        tk.Label(frame_lable_row_2, font=LabelFont, fg=Labelcolor, 
                 bg = 'black', text = 'VBATT').place(x = 120, y= 4)
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # screen title
        tk.Label(frame_lable_Title, font=TitleFont, fg=Labelcolor, 
                 bg = 'black', text = 'ANTRIEB').place(relx = .5, rely =0.5, anchor = 'center')
        # white line in first canvas
        linecanvas_1.create_line(0, 1,240, 1, fill = 'white', width = 2)
        # Oil Temperature:
        tk.Label(frame_lable_row_1, font=LabelFont, fg=Labelcolor, 
...
class PageThree(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.ValPos_2 = tk.IntVar()
        tk.Label(frame_value_row_2, font = ValueFont, fg =Valuecolor,
                 bg = 'black' , textvariable = self.ValPos_2).place(x = 115, y =15, anchor ='e')
        tk.Label(frame_value_row_2, font = UnitFont, fg =Unitcolor,
                 bg = 'black',text = 'm').place(x = 112, y =19, anchor ='w')         
        
        self.GetValPos_1() #And in this case ValPos_2 as well
    
    def GetValPos_1(self):
        #pressure
        self.value = get_signal.GetAirPressure(self)
        self.ValPos_1.set(self.value)
        #Altitude:
        self.QNH = 1013.25 #Standard pressure at sealevel
        # self.altitude = round(((get_signal.GetModuleTemp(self)+273.15)/0.0065)*(pow(self.QNH/self.value,0.190234)-1),1)
        self.altitude = round(((10+273.15)/0.0065)*(pow(self.QNH/self.value,0.190234)-1),1) # 10 should be the actual outside temperature
        self.ValPos_2.set(self.altitude)
...
class get_signal:
    ...
    def GetModuleTemp(self):
        self.data = bme280.sample(smbus2.SMBus(1), 0x76, calibration_params)
        #Temperature
        self.value = round(self.data.temperature,1)
        return self.value
    
    def GetModuleHumid(self):
        self.data = bme280.sample(smbus2.SMBus(1), 0x76, calibration_params)
        #Humidity
